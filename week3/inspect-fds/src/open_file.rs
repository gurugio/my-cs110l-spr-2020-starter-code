...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let path = fs::read_link(&link_path).ok();
        match path {
            Some(p) => {
                let fdinfo_path = format!("/proc/{}/fdinfo/{}", pid, fd);
                let contents = fs::read_to_string(fdinfo_path).unwrap();
                Some(OpenFile::new(
                    String::from(OpenFile::path_to_name(p.to_str().unwrap())),
                    OpenFile::parse_cursor(&contents).unwrap(),
                    OpenFile::parse_access_mode(&contents).unwrap(),
                ))
            }
            None => None,
        }
...
mod test {
    ...
    fn test_openfile_from_fd_invalid_fd() {
        ...
        assert!(
            OpenFile::from_fd(process.pid, 300).is_none(),
